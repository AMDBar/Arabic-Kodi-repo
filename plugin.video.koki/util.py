...
def makeLink2(params, baseUrl=sys.argv[0]):
    """
    Build a link with the specified base URL and parameters
    ...
    BaseURL: the base URL, sys.argv[0] by default
    """
    return baseUrl + '?' +urllib.urlencode(dict([k.encode('utf-8'),(v).encode('utf-8')] for k,v in params.items()))


def addMenuItem(category, link, icon=None, thumbnail=None, folder=True):
    """
    ...
    """
    listItem = xbmcgui.ListItem(unicode(category), iconImage=icon, thumbnailImage=thumbnail)
    listItem.setInfo(type="Video", infoLabels={ "Title": category })
...
def addMenuItem2(caption, link, icon=None, thumbnail=None, folder=False):
    ...
    Returns True if the item is successfully added, False otherwise
    """
    list_Item = xbmcgui.ListItem(caption, iconImage=icon, thumbnailImage=thumbnail)
    list_Item.setInfo(type="Video", infoLabels={ "Title": caption })
    
    return xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=link, listitem=list_Item, isFolder=folder)
